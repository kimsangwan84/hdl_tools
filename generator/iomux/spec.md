...
### 2.3 MUX 제외
- `-mux_exclude <NAME>` 옵션은 여러 번 사용 가능.  
...
2) **Marker** — `C`(Clock) / `R`(Reset). Clock은 **anchor buffer** 필수, Reset은 현재 무시(미구현).
...
- **Enable 동반 표기**: `signal / enable` 형식 허용.  
  ...
  - Active-high: `<signal>_oe`
  - Active-low:  `<signal>_oen`, `<signal>_oe_n`, `<signal>_oen_n`
...
**Body**
- **입력 경로**: 기능 신호가 입력이면  
  `assign <sig> = test_en ? test_in/io[idx].C : <default>;`  
  IN/IO를 입력으로 쓸 때 TI_*(`PE/PS/ST/IE` …)는 `gpio_pkg` 상수로 고정.
- **IO 출력 경로**: 기능 신호가 출력이면  
  `assign test_io[idx].I = <sig>;` **(test_en으로 게이트하지 않음)**  
...
- NAND 체인: 처음 두 입력 NAND → 이후는 직전 출력과 NAND 연쇄. 최종 출력은 모든 출력 IO PAD의 `test_io[*].I`에 연결(복수여도 동일 신호).  
  TI_* / TO_* 규칙 준수.
...
**Body**
- 테스트 모드가 하나라도 켜지면 Scan 우선, 아니면 IPDT, 둘 다 아니면 Normal의 `if_pad_*`를 선택하여 PAD에 연결.
- PAD 인스턴스
  - `primemas_lib_OSC_PAD #(.ORIENTATION("V|H"))`
  - `primemas_lib_IN_PAD  #(.ORIENTATION("V|H"))`
  - `primemas_lib_IO_PAD  #(.ORIENTATION("V|H"))`
- ORIENTATION은 엑셀 `PAD type`의 `_V/_H`에서 추출.
- 인스턴스명은 `u_{kind}_{idx:03d}_pad_{BASE}_{bit:0N}` 형식(최소 3자리 0패딩). 대괄호(`[]`) **금지**.
